        ...
        if (curObj != obj && (curObj->GetDistance2dSq(obj) <= fRange || fRange == 0.0f))
        {
            if (!obj->IsInRangeSet(curObj))
            {
                ...
                if (curObj->IsPlayer())
                {
                    ...
                    if (plObj2->canSeeOrDetect(obj) && !plObj2->IsVisible(obj->GetGUID()))
                    {
                        CHECK_BUF;
                ...
                if (plObj != nullptr)
                {
                    if (plObj->canSeeOrDetect(curObj) && !plObj->IsVisible(curObj->GetGUID()))
                    {
                        CHECK_BUF;
            ...
            else
            {
                ...
                {
                    plObj2 = static_cast<Player*>(curObj);
                    cansee = plObj2->canSeeOrDetect(obj);
                ...
                if (plObj != nullptr)
                {
                    cansee = plObj->canSeeOrDetect(curObj);
...
void MapMgr::ChangeFarsightLocation(Player* plr, DynamicObject* farsight)
{
    ...
    if (farsight == 0)
    {
        ...
        {
            if (plr->IsVisible((*itr)->GetGUID()) && !plr->canSeeOrDetect((*itr)))
            {
            ...
            for (uint32 posY = startY; posY <= endY; ++posY)
            {
                ...
                if (cell != nullptr)
                {
                    for (MapCell::ObjectSet::iterator iter = cell->Begin(); iter != cell->End(); ++iter)
                    {
                        ...
                        if (!plr->IsVisible(obj->GetGUID()) && plr->canSeeOrDetect(obj) && farsight->GetDistance2dSq(obj) <= m_UpdateDistance)
                        {
                            ByteBuffer buf;
