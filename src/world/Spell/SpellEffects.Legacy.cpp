...
pSpellEffect SpellEffectsHandler[TOTAL_SPELL_EFFECTS] =
{
    ...
    &Spell::spellEffectNotImplemented,          //  39 SPELL_EFFECT_UNUSED
...
void Spell::SpellEffectSchoolDMG(uint8_t effectIndex) // dmg school
{
    ...
    {
        m_caster->doSpellDamage(unitTarget, getSpellInfo()->getId(), dmg, pSpellId == 0);
    }
    else
    {
        if (GetType() == SPELL_DMG_TYPE_MAGIC)
        {
            m_caster->doSpellDamage(unitTarget, getSpellInfo()->getId(), dmg, !m_triggeredSpell, static_damage);
        }
        else
        {
            if (u_caster != nullptr)
...
void Spell::SpellEffectApplyAura(uint8_t effectIndex)  // Apply Aura
{
    ...
    if (itr == m_pendingAuras.end())
    {
        ...
        AddRef();
        sEventMgr.AddEvent(this, &Spell::handleAddAura, unitTarget->getGuid(), pAura, EVENT_SPELL_HIT, 100, 1, 0);
    }
    else
    {
        pAura = itr->second;
...
void Spell::SpellEffectEnvironmentalDamage(uint8_t /*effectIndex*/)
{
    ...
    //this is GO, not unit
    m_caster->doSpellDamage(playerTarget, getSpellInfo()->getId(), damage, pSpellId == 0, true);
...
void Spell::SpellEffectWeaponDmgPerc(uint8_t effectIndex) // Weapon Percent damage
{
    ...
    if (GetType() == SPELL_DMG_TYPE_MAGIC)
    {
        ...
        u_caster->doSpellDamage(unitTarget, getSpellInfo()->getId(), dmg, false);
    }
    else
    {
        uint32 _type;
...
void Spell::SpellEffectDispel(uint8_t effectIndex) // Dispel
{
    ...
    for (const auto& aur : unitTarget->getAuraList())
        {
            bool AuraRemoved = false;
            aursp = aur->getSpellInfo();

            if (dispelNegativeAuras)
            {
                if (!aur->isNegative())
                    continue;
            }
...
void Spell::SpellEffectPowerBurn(uint8_t effectIndex) // power burn
{
    ...
    m_caster->doSpellDamage(unitTarget, getSpellInfo()->getId(), static_cast<uint32_t>(mana * getSpellInfo()->getEffectMultipleValue(effectIndex)), pSpellId == 0, true);
}
