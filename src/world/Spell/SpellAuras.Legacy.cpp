...
Aura::Aura(SpellInfo* proto, int32 duration, Object* caster, Unit* target, bool temporary, Item* i_caster)
{
    ...
    if (p_target && getTimeLeft() > 0)
    {
        ...
        setTimeLeft((getTimeLeft() * (100 + DurationModifier)) / 100);
    }
    ...
    if (!IsPassive())
    {
        setMaxDuration(static_cast<int32_t>(duration * m_spellHaste));
    ...
    if (apply)
    {
        ...
        if (!m_target->isStealthed() && m_target->HasAura(58426))
        {
            ...
            if (buff)
            {
                ...
                m_target->sendAuraUpdate(buff, false);
            }
            else
        ...
        if (m_target->isPlayer())   // Vanish
        {
            switch (m_spellInfo->getId())
            {
                ...
                case 71400:
                {
                    ...
                    {
                        if (aur->getSpellInfo()->getMechanicsType() == MECHANIC_ROOTED || aur->getSpellInfo()->getMechanicsType() == MECHANIC_ENSNARED)   // Remove roots and slow spells
                        {
                            aur->removeAura();
                        }
                        else // if got immunity for slow, remove some that are not in the mechanics
                        {
                            for (uint8 i = 0; i < 3; i++)
                            {
                                uint32 AuraEntry = aur->getSpellInfo()->getEffectApplyAuraName(i);
                ...
                for (const auto& aur : m_target->getAuraList())
                {
                    if (aur->getSpellInfo()->getEffectApplyAuraName(0) != SPELL_AURA_DUMMY)
                    {
                        ...
                        if (tmp_duration != 0)
                        {
                            ...
                            m_target->sendAuraUpdate(aur, false);
                            sEventMgr.AddEvent(aur, &Aura::removeAura, EVENT_AURA_REMOVE, tmp_duration, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT | EVENT_FLAG_DELETES_OBJECT);
                        }
                    }
                }
            } break;
    ...
    switch (m_spellInfo->getId())
    {
        ...
        case 69924:
        {
            if (apply)
            {
                ...
                for (const auto& pAura : m_target->getAuraList())
                {
                    if (pAura != this &&
                        pAura != nullptr &&
                        !pAura->IsPassive() &&
...
void Aura::SpellAuraModDispelImmunity(AuraEffectModifier* aurEff, bool apply)
{
    ...
    if (apply)
    {
        for (const auto& aur : m_target->getAuraList())
        {
            ...
            if (aur->getSpellId() != 41425 && aur->getSpellId() != 25771)
                if (aur->getSpellInfo()->getDispelType() == (uint32)aurEff->miscValue)
                    aur->removeAura();
        }
    }
}
        ...
        if (aurEff->miscValue != 16 && aurEff->miscValue != 25 && aurEff->miscValue != 19) // don't remove bandages, Power Word and protection effect
        {
            ...
            if (m_spellInfo->getId() == 42292 || m_spellInfo->getId() == 59752)
            {
                for (const auto& aur : m_target->getAuraList())
                {
                    ...
                    {
                        switch (aur->getSpellInfo()->getEffectApplyAuraName(y))
                        {
...
void Aura::SpellAuraModIncreaseHealthPerc(AuraEffectModifier* aurEff, bool apply)
{
    ...
    if (apply)
    {
        ...
        //      TO< Pet* >(m_target)->SetHealthFromSpell(((Pet*)m_target)->GetHealthFromSpell() + mod->fixed_amount[aurEff->effIndex]);
    }
    else
    {
        m_target->modMaxHealth(-aurEff->mFixedDamage);
...
void Aura::SpellAuraIncreaseHealingByAttribute(AuraEffectModifier* aurEff, bool apply)
{
    ...
    if (apply)
    {
        ...
        //mod->fixed_amount[aurEff->effIndex] = val; //we wish to have the same amount when we are removing the spell as when we were applying !
    }
...
void Aura::SpellAuraSpellHealingStatPCT(AuraEffectModifier* aurEff, bool apply)
{
    ...
    {
        //mPositive = true;
        /*mod->realamount = (aurEff->mDamage * m_target->getStat(aurEff->miscValue) /1 00;
