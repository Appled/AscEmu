...
// MIT End
// APGL Start (from Spell.Legacy.h for testing purpose)
...
(1 << MECHANIC_CHARMED) | (1 << MECHANIC_DISORIENTED) | (1 << MECHANIC_FLEEING) | \
(1 << MECHANIC_ROOTED) | (1 << MECHANIC_ASLEEP) | (1 << MECHANIC_ENSNARED) | \
(1 << MECHANIC_STUNNED) | (1 << MECHANIC_FROZEN) | (1 << MECHANIC_INCAPACIPATED) | \
(1 << MECHANIC_POLYMORPHED) | (1 << MECHANIC_BANISHED) | (1 << MECHANIC_SHACKLED) | \
(1 << MECHANIC_TURNED) | (1 << MECHANIC_HORRIFIED) | (1 << MECHANIC_DAZED) | \
(1 << MECHANIC_SAPPED))
...
class SERVER_DECL Spell : public EventableObject
{
    public:
        ...
        // Checks for shapeshift errors on cast
        SpellCastResult getErrorAtShapeshiftedCast(SpellInfo const* spellInfo, const uint32_t form) const;
        // Checks for caster's range from target
        SpellCastResult checkRange(bool tolerate) const;
        // Checks if caster has enough power to cast spell
        SpellCastResult HasPower() const;
        // Checks if caster has enough runes to cast spell
        SpellCastResult hasRunes() const;
        // Checks if caster is in state where the spell cannot be casted
        SpellCastResult checkCasterState() const;
        // Checks can the spell be casted
        SpellCastResult CanCast(bool tolerate, uint32_t* parameter1 = nullptr, uint32_t* parameter2 = nullptr);
        // Prepares the spell that's going to be casted on targets
        void prepare(SpellCastTargets* targets);

        // Returns the damage type of spell
        uint8_t getSpellDamageType() const;

        // Calculates spell's power cost in Spell::prepare
        int32_t calculatePowerCost() const;

        // Returns spell's cast time
        int32_t getCastTime() const { return m_castTime; }

        SpellInfo const* GetSpellInfo() const { return m_spellInfo; }

        // Checks if spell effect's targetA is positive target
        bool isPositiveSpellEffectTarget(const uint32_t targetA) const
        {
            switch (targetA)
            {
                case EFF_TARGET_SINGLE_FRIEND:
                case EFF_TARGET_SINGLE_PARTY:
                case EFF_TARGET_CHAIN_HEAL:
        ...
        // Checks if spell effect's targetA is negative target
        bool isNegativeSpellEffectTarget(const uint32_t targetA) const
        {
            switch (targetA)
            {
                case EFF_TARGET_CHAIN_DAMAGE:
                case EFF_TARGET_CURRENT_ENEMY_TARGET_COORDINATES:
