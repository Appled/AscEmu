...
bool Aura::canPeriodicEffectCrit()
{
    ...
    for (const auto& aur : caster->mAuraListByEffect[SPELL_AURA_ALLOW_DOT_TO_CRIT])
    {
...
void Aura::refresh([[maybe_unused]]bool saveMods/* = false*/, int16_t modifyStacks/* = 0*/)
{
    ...
    if (getSpellInfo()->getProcCharges() > 0)
    {
        ...
        maxStacks = static_cast<uint32_t>(spellCharges);
    }
    ...
    else
    {
        ...
        setMaxDuration(static_cast<int32_t>(getMaxDuration() * m_spellHaste));
        for (uint8_t i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (m_auraEffects[i].mAuraEffect == SPELL_AURA_NONE)
                continue;
...
void Aura::update(unsigned long diff)
{
    // Do not update trash
    if (m_isGarbage)
        return;
    ...
    if (m_duration > 0)
    {
        if (diff >= static_cast<uint32_t>(m_duration))
        {
            removeAura();
            m_duration = 0;
        }
...
{
    // Reset max duration first to normal
    if (getMaxDuration() > 0 && m_spellHaste != 0.0f)
        setMaxDuration(static_cast<int32_t>(getMaxDuration() / m_spellHaste));
...
void Aura::_calculateEffectAmplitude(uint8_t effIndex)
{
    ...
    if (m_spellHaste != 0.0f)
        m_auraEffects[effIndex].mAmplitude = static_cast<uint32_t>(amplitude * m_spellHaste);
    else
        m_auraEffects[effIndex].mAmplitude = static_cast<uint32_t>(amplitude);
}
...
bool Aura::_canHasteAffectDuration()
{
    ...
    for (const auto& aur : caster->mAuraListByEffect[SPELL_AURA_ALLOW_HASTE_AFFECT_DURATION])
    {
...
void Aura::periodicTick(AuraEffectModifier* aurEff)
{
    switch (aurEff->mAuraEffect)
    {
        case SPELL_AURA_PERIODIC_DAMAGE:
        {
            ...
            const auto casterUnit = GetUnitCaster();
            if (casterUnit != nullptr)
                casterUnit->doSpellDamage(getOwner(), getSpellId(), aurEff->mDamage, true, staticDamage, true, false, false, this, aurEff);
            else
                getOwner()->doSpellDamage(getOwner(), getSpellId(), aurEff->mDamage, true, staticDamage, true, false, false, this, aurEff);
        } break;
        case SPELL_AURA_PERIODIC_HEAL:
        {
            if (!getOwner()->isAlive())
        ...
        case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
        {
            ...
            Spell* spell = sSpellMgr.newSpell(getOwner(), triggerSpell, true, this);
        ...
        case SPELL_AURA_PERIODIC_ENERGIZE:
        {
            ...
            if (casterUnit != nullptr)
                casterUnit->energize(getOwner(), getSpellId(), aurEff->mDamage, powerType);
            else
                getOwner()->energize(getOwner(), getSpellId(), aurEff->mDamage, powerType);
        ...
        case SPELL_AURA_PERIODIC_LEECH:
        {
            ...
            if (casterUnit != nullptr)
                damageInflicted = casterUnit->doSpellDamage(getOwner(), getSpellId(), aurEff->mDamage, true, staticDamage, true, true, false, this, aurEff);
            else
                damageInflicted = getOwner()->doSpellDamage(getOwner(), getSpellId(), aurEff->mDamage, true, staticDamage, true, true, false, this, aurEff);
            ...
            casterUnit->doSpellHealing(casterUnit, getSpellId(), healAmount, true, true, true, true, false, this, aurEff);
        } break;
        case SPELL_AURA_PERIODIC_HEALTH_FUNNEL:
        {
            if (!getOwner()->isAlive())
            ...
            int32_t damageInflicted = 0;
            if (casterUnit != nullptr)
                damageInflicted = casterUnit->doSpellDamage(getOwner(), getSpellId(), aurEff->mDamage, true, staticDamage, true, false, false, this, aurEff);
            else
                damageInflicted = getOwner()->doSpellDamage(getOwner(), getSpellId(), aurEff->mDamage, true, staticDamage, true, false, false, this, aurEff);
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            if (casterUnit != nullptr)
                casterUnit->doSpellDamage(getOwner(), getSpellId(), dmg, pSpellId == 0, staticDamage, true, false, false, this, aurEff);
            else
                getOwner()->doSpellDamage(getOwner(), getSpellId(), dmg, pSpellId == 0, staticDamage, true, false, false, this, aurEff);
        } break;
        case SPELL_AURA_PERIODIC_POWER_BURN:
        {
            ...
            if (casterUnit != nullptr)
                casterUnit->doSpellDamage(getOwner(), getSpellId(), damage, false, staticDamage, true, false, false, this, aurEff);
            else
                getOwner()->doSpellDamage(getOwner(), getSpellId(), damage, false, staticDamage, true, false, false, this, aurEff);
        } break;
        ...
        case SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE:
        {
            ...
            const auto casterUnit = GetUnitCaster();
            if (casterUnit != nullptr)
                casterUnit->castSpell(getOwner(), triggeredInfo, aurEff->mDamage, true);
        } break;
        default:
            break;
    }
