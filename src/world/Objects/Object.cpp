...
int32_t Object::doSpellDamage(Unit* victim, uint32_t spellId, int32_t dmg, bool allowProc/* = true*/, bool staticDamage/* = false*/, bool isPeriodic/* = false*/, bool isLeech/* = false*/, bool forceCrit/* = false*/, Aura* aur/* = nullptr*/, AuraEffectModifier* aurEff/* = nullptr*/)
{
    ...
    if (victim->isPlayer())
    {
        damageReductionPct = static_cast<float_t>(static_cast<Player*>(victim)->CalcRating(PCR_SPELL_CRIT_RESILIENCE) / 100.0f);
        if (damageReductionPct > 1.0f)
            damageReductionPct = 1.0f;
    }
    ...
    if (isPeriodic && aur != nullptr && aurEff != nullptr)
    {
        ...
        if (aur->getTimeLeft() >= static_cast<int32_t>(aurEff->mAmplitude))
        {
            damage_int = static_cast<int32_t>(damage);
            aurEff->mDamageFraction = damage - damage_int;
        }
        else
        {
            // In case this is the last tick, just round the value
            damage_int = static_cast<int32_t>(std::round(damage));
        }
    }
    else
    {
    ...
    auto damage_uint = static_cast<uint32_t>(damage_int);
    auto absorbedDamage = victim->AbsorbDamage(school, &damage_uint);
    const auto manaShieldAbsorb = victim->ManaShieldAbsorb(damage_uint);

    if (manaShieldAbsorb > 0)
    {
        if (manaShieldAbsorb > damage_uint)
            damage_uint = 0;
        else
            damage_uint -= manaShieldAbsorb;

        absorbedDamage += manaShieldAbsorb;
    }

    damage_int = static_cast<int32_t>(damage_uint);
    ...
    {
        const uint32_t overKill = damage_int > healthBefore ? healthBefore - damage_int : 0;
        static_cast<Unit*>(this)->sendPeriodicAuraLog(GetNewGUID(), victim->GetNewGUID(), spellInfo, damage_int, overKill, absorbedDamage, dmginfo.resisted_damage, aurEff->mAuraEffect, isCritical);
    }
    else
    {
        SendSpellNonMeleeDamageLog(this, victim, spellId, damage_int, school, absorbedDamage, dmginfo.resisted_damage, false, 0, isCritical, isPlayer());
    }
    ...
    if (spellInfo->getSchoolMask() & SCHOOL_MASK_SHADOW)
    {
        if (victim->isAlive() && isCreatureOrPlayer())
        {
            ...
            if (spellId == 32379 || spellId == 32996 || spellId == 48157 || spellId == 48158)
            {
                ...
                SendSpellNonMeleeDamageLog(this, this, spellId, backfireAmount, school, absorbed, 0, false, 0, false, isPlayer());
            }
        }
    }
    ...
    if (isLeech)
        return std::min(damage_int, static_cast<int32_t>(healthBefore));
    else
        return damage_int;
}
...
void Object::doSpellHealing(Unit* victim, uint32_t spellId, int32_t amt, bool allowProc/* = true*/, bool staticDamage/* = false*/, bool isPeriodic/* = false*/, bool isLeech/* = false*/, bool forceCrit/* = false*/, Aura* aur/* = nullptr*/, AuraEffectModifier* aurEff/* = nullptr*/)
{
    ...
    if (heal > 0.0f)
    {
        ...
        if (isCritical)
        {
            heal = static_cast<Unit*>(this)->GetCriticalHealBonusForSpell(victim, spellInfo, heal);
    ...
    // but skip leech spells because the damage part only uses fractions
    if (isPeriodic && !isLeech && aur != nullptr && aurEff != nullptr)
    {
        heal += aurEff->mDamageFraction;
        if (aur->getTimeLeft() >= static_cast<int32_t>(aurEff->mAmplitude))
        {
            heal_int = static_cast<int32_t>(heal);
            aurEff->mDamageFraction = heal - heal_int;
        }
        else
        {
            // In case this is the last tick, just round the value
            heal_int = static_cast<int32_t>(std::round(heal));
        }
    }
    else
    {
    ...
    if (isPeriodic && aurEff != nullptr && isCreatureOrPlayer())
    {
        ...
        static_cast<Unit*>(this)->sendPeriodicAuraLog(GetNewGUID(), victim->GetNewGUID(), spellInfo, heal_int, overHeal, absorbedHeal, 0, aurEff->mAuraEffect, isCritical);
    }
    else
    {
        Spell::SendHealSpellOnPlayer(this, victim, heal_int, isCritical, overHeal, spellId);
    }
